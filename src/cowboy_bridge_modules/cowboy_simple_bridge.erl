...
init({Req, DocRoot}) ->
	...
	trace_utils:debug_fmt( "Initialising Cowboy simple bridge with:~n"
		" - Req = '~p'~n - DocRoot = '~p'", [ Req, DocRoot ] ),

	%basic_utils:crash(),

	ReqKey = new_key(),
	put_key(ReqKey, #request_cache{ body = not_loaded, docroot=DocRoot,
									request = Req}),
	ReqKey.
